...
/*
 * xml 模板解析器，通过输入的xml规则描述文件，抽取目标html文档的元素信息，返回一个JSON字符串
 * xml元素解释：
 * 	name属性：在当前作用域内节点标识，下一级默认scope，同时提取成功的JSON串的key值为由name定义
 * 	xpath:描述当前节点的位置或者值，（如当前节点不是最终节点，那么该xpath描述节点定位规则，如果当前节点没有子节点，那么描述的是一个文本值），xpath使用htmlcleaner解析，请使用htmlcleaner支持的xpath语法
 * 	regex:文本抽取规则，为正则表达式，底层调用java正则，故请使用java支持的正则语法，另外规则使用group 1，如果作全文匹配，请使用圆括号包围
 * 		  regex将会作为数据是否提取成功的标识
 * 	test:测试数据是否提取成功，如果没有成功，test包围的属性将不会被录入。如果提取成功，数据将会设置到test的scope中，同时后续相同group的test将会被忽略，测试失败条件是：property的require属性不为true（默认为false），或者xpath没有成功提取，如果存在正则表达式，那么正则表达式提取或者匹配成功
 * 	group：用在test中，表示test的分组，默认group为0，在相同group中，如果有一个提取成功，那么后续提取方案将会被忽略。
 *  scope：值存储空间，或者作用域。为一个树形结构关系，默认每一个匹配单元的scope由父节点的name定义，提取成功后的数据将会存放在scope中，fetch节点（文档根节点）的默认name为page，也及在默认情况下提取单元可以强行将scope设置为page，那么对于数据将会存放在全局中，供其它模块调用
 *  fetch:默认为false，如果为true，将会收集本作用域的值，并在fetch方法的返回值的存入
 *  check:如果设置check熟悉为true，那么本条记录仅仅是作为上下文判断的一个标记，提取的值不会存入。check主要设计为和test，value配合，三则配合可以实现存入xml自定义的值。
 *  如：html中对于性别的定义是男、女。我想将最终结果中录入的性别设置为美女，帅哥。那么可以使用如下代码
 *  <test group="3543654">
 *   <property name="genderCheck" check="true" xpath="xxx" regex=".*(男).*"/>
 *   <property name="gender" value="帅哥"/>
 *  </test>
 *  这样，如果按照html中抽取出了男这个词，将会在最终结果集中写入”帅哥“
 *  
 *  value:用户自定义value，如果用户指定了value，那么解析模块将不会执行，直接采纳value值
 *  
 * 备注：由于类似网页的布局可能存在局部差异，所以一个特定的模板可能不能完整的提取所有数据，为提高数据完整性，可以通过以下方法
...
public class XmlModeFetcher {
	...
	private void init(String xml) throws DocumentException{
		...
		while(regMatcher.find()){
			...
			//System.out.println(i+++"  : "+strReg.length()+"  "+strReg);
	...
	private boolean setNodeDataValue(String name,String value,NodeData node,String scope,HashMap<String,ScopeValue> failedValues){
		if(node == null){
	...
	private boolean innerFetch(List<String> container, Element element ,NodeData parent,NodeData root,TagNode tagnode,HashMap<String,ScopeValue> failedValue){
		...
		if(classfetcher != null){
			...
			if(!this.classFetchers.containsKey(classfetcher)){
				try {
					classFetchers.put(classfetcher, (ClassFetcher) XmlModeFetcher.class.getClassLoader().loadClass(classfetcher).getDeclaredConstructor().newInstance());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					return false;
		...
		if(element.getName().equals("fetch")){//root
			...
			if(istestSuccessful){
				...
				while(itfailedValues.hasNext()){
					...
					setNodeDataValue(itemvalue.getKey(), itemvalue.getValue().value, parent, itemvalue.getValue().scope, null);
				}
			...
			try {
				...
				if(textObjs.length ==0){
					//logger.info("fetch failed for xpath:"+xpath+"  parent xpath:"+element.getParent().attributeValue("xpath",""));
